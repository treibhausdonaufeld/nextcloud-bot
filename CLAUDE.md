...
- **Controllers** (`app/controllers/`) are synchronous handlers (Ravyn runs them in a threadpool): dashboard (FTS search), groups (vis-network org chart), timeline (plotly from markdown tables on a Collectives page), protocols, logbook (decision cards + XLSX import), members (role overview + role history), mentions (table + network graph). Graph pages fetch JSON endpoints (`/groups/graph.json`, `/mentions/graph.json`) and load click-detail partials via htmx. Every detail partial (member, role, group, user) is swapped into the shared `#detail` target of `partials/detail_dialog.html`, which pages include to get the modal overlay and its open/close wiring — so a badge in one dialog can link straight into another. Role badges come from the `role_badge` macro (`partials/role_badge.html`) and carry two destinations in one pill: the group half opens that group's dialog (with a real `/groups?limit_group=…` href behind it, so opening it in a new tab still works), the role half the list of everybody holding it. A node naming neither a group nor a known user — the group half of a past role whose page has been retired — renders an explicit empty state rather than a blank user card. The org chart draws groups only by default (`with_members` is opt-in): clicking a group opens its dialog, which lists the members with the role each holds, using the same `role-badge role-<role>` classes and group hue as `/members`, plus the `Karenz` badge for anybody currently on leave.
