...
class AvatarFetcher:
    ...
    def fetch_avatar(self, username: str):
        ...
        try:
            ...
            # save to dot_jpg path only if it's different from the primary path
            ...
            logger.error("Error converting avatar image for %s: %s", username, e)

    @staticmethod
    def _save_as_jpeg(content: bytes, path: Path) -> None:
